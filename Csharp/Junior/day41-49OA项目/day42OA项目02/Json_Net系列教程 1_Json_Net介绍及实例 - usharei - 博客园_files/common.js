...
function cb_CodeHighlight() {
    SyntaxHighlighter.config.strings.viewSource = "View Code";
...
function blogCommentManager() {
    ...
    var scrollCommentForm = function () {
        if (window.location.hash == "#commentform") {
            var scrollTo = $('#comment_form_container');
            $('html, body').animate({
    ...
    this.renderComments = function (pageIndex) {
        var ajaxParam = {
            postId: cb_entryId,
            ...
            success: function (data) {
                if (data) {
                    ...
                    if (data.commentCount > 0) {
                        $("#blog-comments-placeholder").html(data.commentsHtml);
...
function RefreshCommentList() {
    ...
    if (!comment_maxDate) {
        comment_maxDate = $("#post-date").html();
    }
...
function loadViewCount(postId) {
    $.get('/mvc/blog/ViewCountCommentCout.aspx?postId=' + postId, function (data) {
        $('#post_view_count').html(data)
    })
}
...
function runJsCode(id) {
    ...
    var runWindow = window.open("about:blank", 'runWindow'); 
